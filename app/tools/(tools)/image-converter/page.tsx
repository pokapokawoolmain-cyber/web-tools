...
const faqSchema = {
  ...
  mainEntity: [
    ...
    { "@type": "Question", name: "画質を落とさずに変換できますか？", acceptedAnswer: { "@type": "Answer", text: "はい。「画質そのまま（無劣化）」プリセットを選ぶと、PNG形式・寸法変更なしで1ピクセルも画質を落とさず変換できます（可逆圧縮のため元画像と完全に同じ画素データになります）。JPG・WebP・AVIFで軽量化したい場合も、品質を高め（90以上）に設定すれば見た目の劣化をほぼ抑えられます。また、縮小・拡大の際は高品質な補間処理を使用しており、画像がにじんだりギザギザになったりするのを防いでいます。" } },
...
const seoContent = (
  <div className="space-y-8 text-sm text-slate-600 dark:text-slate-400 leading-relaxed">
    ...
    <section>
      ...
      <p className="mt-3">
        <strong>絶対に画質を落としたくない場合</strong>は「画質そのまま（無劣化）」プリセットをお使いください。PNG・寸法変更なしで変換され、
        元の画素データと完全に同じ結果になります（ピクセル単位で無劣化であることを検証済みです）。また、リサイズを行う場合も、
        当ツールは高品質な補間処理（バイリニア/バイキュービック相当）を使用しており、単純な縮小・拡大でありがちな
        「ギザギザ」や「にじみ」を最小限に抑えています。
      </p>
    </section>
