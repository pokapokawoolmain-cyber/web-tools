...
type Sig = {
  ...
  dataUrl?: string;
  text?: string;
  x: number;    // canvas pixels from left (0..cw)
  y: number;    // canvas pixels from top (0..ch)
  w: number;    // canvas pixels width (for seals: always == h)
  h: number;    // canvas pixels height (for seals: always == w)
  rot: number;  // rotation degrees
  opa: number;  // opacity 0–1
};
...
export function PdfSignature() {
  ...
  // Refs to avoid stale closures in imperative handlers
  ...
  const resizingRef = useRef<{ sigId: string; origW: number; origH: number; startX: number; startY: number; square: boolean } | null>(null);
  ...
  // Imperative pointer events (required for mobile preventDefault)
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      e.preventDefault();
      if (modeRef.current !== "draw") return; // sig divs handle drag
    ...
    const onMove = (e: PointerEvent) => {
      ...
      if (draggingRef.current) {
        ...
        const pos = getPos(e);
        setSigs(ss => ss.map(s => s.id === sigId
          ? { ...s, x: Math.max(0, Math.min(cwRef.current - s.w, pos.x - offX)), y: Math.max(0, Math.min(chRef.current - s.h, pos.y - offY)) }
          : s));
        return;
      }

      if (resizingRef.current) {
        const { sigId, origW, origH, startX, startY, square } = resizingRef.current;
        const pos = getPos(e);
        const dx = pos.x - startX;
        const dy = pos.y - startY;
        const newW = Math.max(20, origW + dx);
        const newH = square ? newW : Math.max(20, origH + dy);
    ...
    const onUp = () => {
      ...
      if (isDrawingRef.current) {
        ...
        const dc = drawCanvasRef.current;
        const path = drawPathRef.current;
        if (!dc || path.length < 2) { drawPathRef.current = []; return; }
  ...
  const placeSeal = (seal: SavedSeal) => {
    ...
    const newSig: Sig = {
      ...
      w: size,
      h: size, // ALWAYS equal to w for seals
      rot: 0,
      opa: 0.9,
    };
    const next = [...sigs, newSig];
    ...
    try {
      ...
      for (const sig of sigs) {
        ...
        if ((sig.type === "draw" || sig.type === "seal") && sig.dataUrl) {
          ...
          if (sig.type === "seal") {
            // Force square - prevents ANY floating point stretch
  ...
  return (
    <div className="space-y-5">
      ...
      {!pdfFile ? (
        ...
        /* PC: left=PDF(wider), right=panel. Mobile: top=PDF, bottom=panel */
          ...
          <div className="w-full lg:flex-1">
            <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-700 overflow-hidden">
              ...
              >
                ...
                {sigs.map(sig => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {sig.type === "text" && (
                        <span style={{ fontSize: 14, color: "#1e3a8a", fontFamily: "serif", whiteSpace: "nowrap", lineHeight: 1, pointerEvents: "none" }}>
                          {sig.text}
                        </span>
                      )}
                      {/* Resize handle */}
                      {selectedId === sig.id && mode !== "draw" && (
                        <div
                          style={{
                            position: "absolute",
                            right: -6,
                ...
                {/* Draw canvas (top layer, only pointer-active in draw mode) */}
                <canvas
                  ref={drawCanvasRef}
                  style={{
                    position: "absolute",
          ...
          <div className="w-full lg:w-72 space-y-4">
            ...
            <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-slate-200 dark:border-zinc-700 p-4 space-y-3">
              <div className="flex gap-2">
                ...
                <button onClick={() => { setMode("seal"); setSavedSeals(getSavedSeals()); }}
                  className={`flex-1 px-3 py-2 rounded-lg text-xs font-semibold border transition-all ${
              ...
              <div className="bg-white dark:bg-zinc-900 rounded-2xl border border-blue-200 dark:border-blue-800/50 p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-blue-600 dark:text-blue-400">
                    {selectedSig.type === "seal" ? "🔴 印鑑" : selectedSig.type === "draw" ? "✍️ 署名" : "💬 テキスト"}
                  </span>
                  <button onClick={deleteSelected} className="text-xs text-red-500 hover:text-red-700 font-medium">
                    🗑 削除
                  </button>
                </div>
                ...
                <div>
                  <div className="flex justify-between text-xs text-slate-500 mb-1">
                    <span>回転</span><span>{selectedSig.rot}°</span>
                  </div>
                  <input type="range" min="-180" max="180" value={selectedSig.rot}
                    onChange={e => setSigs(ss => ss.map(s => s.id === selectedId ? { ...s, rot: +e.target.value } : s))}
                ...
                <div>
                  <div className="flex justify-between text-xs text-slate-500 mb-1">
                    <span>濃さ</span><span>{Math.round(selectedSig.opa * 100)}%</span>
                  </div>
                  <input type="range" min="10" max="100" value={Math.round(selectedSig.opa * 100)}
                    onChange={e => setSigs(ss => ss.map(s => s.id === selectedId ? { ...s, opa: +e.target.value / 100 } : s))}
                    className="w-full accent-blue-600" />
                </div>
              </div>
            )}
